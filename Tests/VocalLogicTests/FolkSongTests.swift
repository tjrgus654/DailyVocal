...
final class FolkSongTests: XCTestCase {
    ...
    func testLibraryShipsTwentyThreeSongs() {
        XCTAssertEqual(VocalLogic.folkSongs.count, 23)
        XCTAssertEqual(VocalLogic.folkSongs.map(\.title),
                       ["아리랑", "강강술래", "한오백년", "정선아리랑", "둥당기타령", "도라지타령", "난봉가", "매화타령", "신고산타령", "오죽령", "닐리리야", "새타령", "변강쇠타령", "쾌지나칭칭나네", "진도아리랑", "오돌또기", "베틀가", "칭칭야", "어랑타령", "돈돌라", "범피중류", "쑥대머리", "성주풀이"])
    ...
    func testRegionTagsAndFilter() {
        ...
        XCTAssertEqual(VocalLogic.songs(inRegion: "강원").map(\.title), ["정선아리랑", "어랑타령"])
        XCTAssertEqual(VocalLogic.songs(inRegion: "전라").map(\.title), ["한오백년", "새타령", "변강쇠타령", "베틀가"])
        XCTAssertEqual(VocalLogic.songs(inRegion: "경상").map(\.title), ["쾌지나칭칭나네", "돈돌라", "성주풀이"])

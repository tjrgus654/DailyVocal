...
const folk = await page.evaluate(`(() => {
  ...
  return {
    three: FOLK_SONGS.length === 20 && FOLK_SONGS.map(s => s.title).join(",") === "아리랑,강강술래,한오백년,정선아리랑,둥당기타령,도라지타령,난봉가,매화타령,신고산타령,오죽령,닐리리야,새타령,변강쇠타령,쾌지나칭칭나네,진도아리랑,오돌또기,베틀가,칭칭야,어랑타령,돈돌라",
...
ok("folk library ships 20 songs", folk.three);

...
public enum VocalLogic {
    ...
    public static func recommendNextGame(
        ...
        passaggioAccuracy: Int? = nil, stretchAccuracy: Int? = nil,
        lastGame: GameType?
    ) -> GameType {
        ...
        scores.append((.stretch, stretchAccuracy ?? 50))
    ...
    // MARK: - Spectral analysis (Goertzel magnitude spectrum)

    /// 512-bin magnitude spectrum via Goertzel over the first 1024 samples
    /// (Hann-windowed). Bins cover 0...(sampleRate/2). Pure logic so the
    /// macOS pipeline integration test can exercise the real DSP on
    /// synthesized voice buffers without a microphone.
    public enum SpectralAnalysis {
        public static func magnitudeSpectrum(_ samples: [Float], sampleRate: Double, binCount: Int) -> [Double] {
    ...
    public static let folkSongs: [FolkSong] = [
        ...
        FolkSong(
            title: "변강쇠타령",
            origin: "전라 민요(판소리 계열) — 전통 원형",
            firstLyric: "변강쇠 죽어서 누구를 타이른다",
            notes: [
        ...
        FolkSong(
            title: "오돌또기",
            origin: "경기 민요(잡가 계열) — 전통 원형",
            firstLyric: "오돌또기 오돌또기",
            notes: [
                SongNote(3, 1), SongNote(3, 1), SongNote(0, 1), SongNote(3, 1),
                SongNote(5, 1), SongNote(5, 2),
                SongNote(3, 1), SongNote(5, 1), SongNote(7, 1), SongNote(5, 1),
                SongNote(3, 4),
            ]
        ),
